...
// Check if the command is on cooldown
  if (commandCooldowns[user.id] && commandCooldowns[user.id][commande] && commandCooldowns[user.id][commande] > Date.now()) {
    const remainingTime = (commandCooldowns[user.id][commande] - Date.now()) / 1000;
    return interaction.reply({ content: `Please wait ${remainingTime.toFixed(1)} more second(s) before using the ${commande} command again.`, ephemeral: true });
  }
        

  // Set the command on cooldown for the specified duration
  const cooldownTime = 5 * 1000; // 10 seconds
  if (!commandCooldowns[user.id]) commandCooldowns[user.id] = {};
  commandCooldowns[user.id][commande] = Date.now() + cooldownTime;
  setTimeout(() => {
    if (commandCooldowns[user.id]) delete commandCooldowns[user.id][commande];
  }, cooldownTime);
