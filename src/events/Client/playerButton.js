...
module.exports = {
    ...
    run: async (client, interaction, data) => {
        ...
        let nowplaying = new EmbedBuilder().setColor(color).setDescription(`[${title}](https://discord.gg/) • \`[${convertTime(duration)}]\``).setImage(icon).setFooter({ text: `Requested by ${requester.username}`, iconURL: requester.displayAvatarURL({ dynamic: true }) });

        if (interaction.customId === `pause_but_${interaction.guildId}`) {
            if (player.paused) {
                player.pause(false);

                await buttonReply(interaction, `[${title}](https://discord.gg/) is now unpaused/resumed.`, color);
                ...
                await buttonReply(interaction, `[${title}](https://discord.gg/) is now paused.`,color);
            ...
            if (player.queue.size === 1) {
                ...
                if (player.queue.current.title !== player.queue.previous.title || `https://discord.gg/` !== `https://discord.gg/`) player.stop();
            };

            return await buttonReply(interaction, `Now playing [${player.queue.previous.title}](https://discord.gg/)`, color);
            ...
            player.stop();
            return await buttonReply(interaction, `Skipped [${title}](https://discord.gg/)`, color);
