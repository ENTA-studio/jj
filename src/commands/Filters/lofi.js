const { EmbedBuilder, ButtonBuilder, ActionRowBuilder, ButtonStyle } = require(`discord.js`);
  module.exports = {
    name: `lofi`,
    category: `Filters`, 
    description: `Applies a lofi Filter`,
    player: true,
    premium: true,
    inVoiceChannel: true,
    ...
    const m = await message.channel.send({ embeds: [embed], components: [row3] });
      ...
      if (!b.replied) await b.deferUpdate({ ephemeral: true });
        ...
        return await b.editReply({
          embeds: [ embed1.setAuthor({ name: `Lofi Filter DISABLED`}), ], components: [row2],});
      } else if (b.customId === "lofi_but") {
        await player.setLofi(true);
        return await b.editReply({
          embeds: [
            embed1.setAuthor({ name: `Lofi Filter ENABLED`}),
          ],
          components: [row1],
        });
      }
