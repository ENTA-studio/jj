const { EmbedBuilder, ButtonBuilder, ActionRowBuilder, PermissionFlagsBits, Permissions, ButtonStyle, StringSelectMenuBuilder } = require("discord.js");
    ...
    switch (s.loadType) {
        ...
        case "TRACK_LOADED":
            ...
            const embed = new EmbedBuilder()
             .setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[0].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)
         ...
         case "SEARCH_RESULT":
             ...
             const results = s.tracks.slice(0, 5).map(x => `• ${index++} | [${x.title}](https://discord.gg/5F4EywGBvu) \`${convertTime(x.duration)}\``)
                    .join("\n");
            ...
            collector.on("collect", async (b) => {
                ...
                if(b.customId === "s_one") {
                    player.queue.add(s.tracks[0]);
                        ...
                        if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[0].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)]})
                } else if(b.customId === "s_two") {
                    player.queue.add(s.tracks[1]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[1].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[1].duration, true)}\` • ${s.tracks[1].requester}`)]})
            
                } else if(b.customId === "s_three") {
                    player.queue.add(s.tracks[2]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[2].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[2].duration, true)}\` • ${s.tracks[2].requester}`)]})
            
                } else if(b.customId === "s_four") {
                    player.queue.add(s.tracks[3]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[3].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[3].duration, true)}\` • ${s.tracks[3].requester}`)]})
            
                } else if(b.customId === "s_five") {
                    player.queue.add(s.tracks[4]);
                    if(player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                    if(msg) await msg.edit({embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[4].title}](https://discord.gg/5F4EywGBvu) \`${convertTime(s.tracks[4].duration, true)}\` • ${s.tracks[4].requester}`)]})
