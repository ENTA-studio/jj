...
function neb(embed, player, client) {
  ...
  return embed
    .setDescription(
      `[${player.queue.current.title}](https://discord.gg/) by ${player.queue.current.author} • \`[${convertTime(player.queue.current.duration)}]\``
    )
    .setImage(icon)
    .setFooter({
      ...
      .send({
        embeds: [
          new EmbedBuilder()
            .setColor(message.client.embedColor)
            .setDescription(
              `Added [${s.tracks[0].title}](https://discord.gg/) to the queue.`
            ),
        ],
      })
      .then((a) =>
  ...
  try {
    ...
    if (!message) {
      let embed1 = new EmbedBuilder()
        ...
        .setDescription(
          `[${track.title}](https://discord.gg/) by ${track.author} - \`[${convertTime(
            track.duration
          )}]\``
        )
        .setImage(icon)
      ...
      let embed2 = new EmbedBuilder()
        .setColor(message.client.embedColor)
        .setDescription(
          `${emojiplay} **Started Playing** - [${track.title}](https://discord.gg/) by ${track.author} - \`[${convertTime(track.duration)}]\``
        )
        .setImage(icon)
        .setFooter({
